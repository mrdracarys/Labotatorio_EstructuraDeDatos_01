...
public class Discotienda
{
    ...
    /**
     * Registra en el archivo de log del programa toda la informacion referente a una excepci�n, ocurrida durante el proceso de persistencia
     * @param excepcion es la excepsion que contiene la informacion del error
     */
    ...
    // -----------------------------------------------------------------
    // Puntos de extension 1
    // -----------------------------------------------------------------
    
    /*
     * Generar un informe de discos en la tienda
     * @throws FileNotFoundException - cuando no existe la ruta especifica del archivo
     */
    ...
    public void generarInformeDiscos() throws FileNotFoundException{
    	...
    	// creamos pluma para escribir el archivo
    	...
    	//Recorrido total de discos
    	for(int i=0;i<discos.size();i++) {
    		//Extraer informacion de los discos
    		Disco miDisco = (Disco)discos.get(i);
    		
    		//Escribimos con la pluma la informacion requerida
    ...
    // -----------------------------------------------------------------
    // Puntos de extension 2
    // -----------------------------------------------------------------
    
    /*
     * Generar un informe de los discos costosos
     * @throws FileNotFoundException - cuando no existe la ruta especifica del archivo
     */
    
    public void discosCostosos(String uGenero) throws FileNotFoundException, IOException
    {
    	int generoExsistente = 0;
    	int discoCostoso = 0;
    	
    	for (int i = 0; i<discos.size(); i++) {
    		
    		// Estraer la informacion de cada disco
    		Disco miDisc = (Disco) discos.get(i);
    		
    		// Buscar si existe el genero dado por el usuario
    		if (miDisc.darGenero().equals(uGenero)) {
    			generoExsistente++;
    	    // Buscar si existen discos con precio superior a 1000
    			if (miDisc.darPrecioDisco()>1000) {
    				discoCostoso++;
    			}
    		}
    	}
    	// Errores//Exceppciones
    	if (generoExsistente==0) {
    		throw new IOException("Genero no encontrado" + uGenero);
    	}else if (discoCostoso==0){
    		throw new IOException("No existen discos con ese costo en el genero " + uGenero);
    	}
    	...
    	// Dar reporte de discos costosos con los datos especificados
    	for (int i = 0; i<discos.size(); i++) {
    		
    		// Estraer la informacion de los discos
    		Disco miDisc = (Disco) discos.get(i);
    		
    		// Evalua el genero ingresado por el ususario
    		if (miDisc.darGenero().equals(uGenero)) {
    			if (miDisc.darPrecioDisco()>1000) {
    				// Escribir con la pluma la informacion requerida
    				pluma.println("Nombre: " + miDisc.darNombreDisco());
    				pluma.println("Artista: " + miDisc.darArtista());
    				pluma.println("Genero: " + miDisc.darGenero());
    				pluma.println("-----------------------------------");
    			}
    		}
    	}
    ...
    public String metodo1( )
    {
        try {
        	...
        	return "El reporte se ha generado de manera correcta :)";
		} catch (Exception e) {
    ...
    public String metodo2( )
    {
    	try {
    		String mensaje = "";
    		String usuario = JOptionPane.showInputDialog(null, "Ingrese el genero", "Discos Costosos", JOptionPane.QUESTION_MESSAGE);
    		if (usuario != null) {
    			discosCostosos(usuario);
				mensaje = "El reporte se ha generado de manera correcta :)";
			}else {
				mensaje = "Gracias :)";
			}
    		return mensaje;
